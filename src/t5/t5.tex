...
\subtitle{Tutorial 4: Normalisation}
...
(a) Is $R$ with $\Sigma$ in 2NF?\\\vspace{5pt}
\textbf{Solution}:
...
(b) Is $R$ with $\Sigma$ in 3NF?\\\vspace{5pt}
\textbf{Solution}:
Let us look at the non-trivial functional dependencies of the form $X \rightarrow \{A\}$ derived from $\Sigma$. Namely after removing the trivial functional dependencies after step 1 of the minimal cover algorithm. Equivalently, we could use a minimal cover.\\\vspace{3pt}
...
(c) Is $R$ with $\Sigma$ in BCNF?\\\vspace{5pt}
\textbf{Solution}:
...
$R_{1.2} = \{A, B, C\}$ with $\Sigma_{1.2} = \{\{A\} \rightarrow \{B\}, \{A\} \rightarrow \{C\}, \{B\} \rightarrow \{A\}\}$.

$R_{1.1}$ with $\Sigma_{1.1}$ is in BCNF.

$R_{1.2}$ with $\Sigma_{1.2}$ is in BCNF.

The result is a decomposition into three fragments: $R_2$, $R_{1.1}$ and $R_{1.2}$. It is the same result as the synthesis.\\
...
\textbf{Solution}:
Incidentally yes, the algorithm \textcolor{red}{\textbf{does not}} guarantee that the result is dependency preserving.
\end{frame}
